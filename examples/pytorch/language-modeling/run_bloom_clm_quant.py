...
# NOTE: quanted model implementation
from transformers.models.bloom.modeling_bloom_quant import BloomForCausalLM
...
# Pruning modules
from sparseopt.sparseopt.pruner.bbs import Prune

# Regularization modules
from compressor_trainer.src.utils.clip_regularization import clip_reg_losses, clip_attention_mask_hook, clip_reg_loss_hook

CLIP_REG_CFG= {
    'eps': -20.0,
    'gamma': 43.0,
    'clip_reg_loss_weight': 1.0
}
# ALL of hook handles, a handle can be used to remove the added hook by calling: ``handle.remove()``
CLIP_REG_HOOKS = []
# Record which tensor(in module) has been registered clip reg hook
CLIP_REG_TENSORS = []

# Quantization modules
from compressor_trainer.src.utils.quantization import qr_losses, true_softmax, \
    qr_hook_for_linear, qr_hook_for_matmul, qr_hook_for_residual

QR_CFG = {
    'qr_loss_weight': 0.1,
    'matmul_per_layer': True,
    'ignore_modules': ('lm_head',),
    'residual_modules': ('dense_dropout_add',)
}
# ALL of hook handles, a handle can be used to remove the added hook by calling: ``handle.remove()``
QR_HOOKS = []
# Record which module has been registered qr hook
QR_MODULES = []
...
def parse_args():
    ...
    parser.add_argument(
        "--quant",
        action="store_true",
        help="Whether to do quantization training."
    )
    parser.add_argument(
        "--qr_loss_weight",
        type=float,
        default=None,
        help="Weight of quantization loss."
    )
    ...
    parser.add_argument(
        "--clip_reg",
        action="store_true",
        help="Whether to do clip regularization."
    )
    parser.add_argument(
        "--clip_reg_loss_weight",
        type=float,
        default=None,
        help="Weight of clip regularization loss."
    )
...
def register_qr_hooks(model: nn.Module):
    # Register forward hook for computing quantization losses
    # 'register_forward_hook()' will return a handle
    for name, module in model.named_modules():
        if name in QR_CFG['ignore_modules'] or type(module) in QR_CFG['ignore_modules']:
            continue

        # Linear layer
        if isinstance(module, nn.Linear):
            is_residual = False
            for kw in QR_CFG['residual_modules']:
                if kw in name:
                    is_residual = True
                    break

            if not is_residual:
                QR_MODULES.append(name)
                QR_HOOKS.append(module.register_forward_hook(qr_hook_for_linear(name)))
        # Matrix multiply
        elif module.__class__.__name__ in ('BaddBMMModule', 'BMMModule'):
            QR_MODULES.append(name)
            QR_HOOKS.append(
                module.register_forward_hook(qr_hook_for_matmul(name, per_layer=QR_CFG['matmul_per_layer']))
            )
        # Module with residual
        else:
            for kw in QR_CFG['residual_modules']:
                if name.endswith(kw):
                    for sub_module in module.modules():
                        if isinstance(sub_module, nn.Linear):
                            QR_MODULES.append(name)
                            QR_HOOKS.append(module.register_forward_hook(qr_hook_for_residual(name, sub_module)))

                            # Note: if there are other target sub-modules, u should comment this line
                            break


def register_clip_reg_hooks(model: nn.Module):
    for name, module in model.named_modules():
        if module.__class__.__name__ == 'ApplyAttnMask':
            CLIP_REG_HOOKS.append(
                module.register_forward_pre_hook(
                    clip_attention_mask_hook(eps=CLIP_REG_CFG['eps'])
                )
            )
            CLIP_REG_HOOKS.append(
                module.register_forward_hook(clip_reg_loss_hook(f'{name}.attention_score', gamma=CLIP_REG_CFG['gamma']))
            )
            CLIP_REG_TENSORS.append(f'{name}.attention_score')


def remove_qr_hooks():
    for hook in QR_HOOKS:
        hook.remove()
...
def remove_clip_reg_hooks():
    for hook in CLIP_REG_HOOKS:
        hook.remove()

    CLIP_REG_HOOKS.clear()
    CLIP_REG_TENSORS.clear()
...
def main():
    ...
    if args.quant:
        # Init to be an empty dict
        qr_losses.clear()
        if args.qr_loss_weight:
            QR_CFG.update(qr_loss_weight=args.qr_loss_weight)
        
        logger.info("Registering quantization hooks..")
        register_qr_hooks(unwrapped_model)
        logger.info("Done!\n")

        logger.info("NOTE: quantization training enabled")
        logger.info(f"Quant config: {QR_CFG}")
        logger.info(f"Quanted modules: {QR_MODULES}\n")
    
    # Clip regularization training
    if args.clip_reg:
        # Init to be an empty dict
        clip_reg_losses.clear()
        if args.clip_reg_loss_weight:
            CLIP_REG_CFG.update(clip_reg_loss_weight=args.clip_reg_loss_weight)
        
        logger.info("Registering clip regularization hooks..")
        register_clip_reg_hooks(unwrapped_model)
        logger.info("Done!\n")

        logger.info("NOTE: clip regularization training enabled")
        logger.info(f"Clip reg config: {CLIP_REG_CFG}")
        logger.info(f"Clip reg tensors: {CLIP_REG_TENSORS}")
    ...
    if not args.eval_only:
        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(model):
                    ...
                    # Quant
                    if args.quant:
                        qr_loss = true_softmax(list(chain(*qr_losses.values())))
                        loss = loss + QR_CFG['qr_loss_weight'] * qr_loss
                        qr_loss = qr_loss.item()
                        qr_loss_details = {
                            module_name: [l.item() for l in ls]
                            for module_name, ls in qr_losses.items()
                        }

                        qr_losses.update({module_name: [] for module_name in QR_MODULES})
                    
                    # Clip regularization
                    if args.clip_reg:
                        clip_reg_loss = sum(chain(*clip_reg_losses.values()))
                        loss = loss + CLIP_REG_CFG['clip_reg_loss_weight'] * clip_reg_loss
                        clip_reg_loss = clip_reg_loss.item()
                        clip_reg_loss_details = {
                            tensor_name: [l.item() for l in ls]
                            for tensor_name, ls in clip_reg_losses.items()
                        }

                        clip_reg_losses.update({tensor_name: [] for tensor_name in CLIP_REG_TENSORS})
                ...
                if step % (100 * args.gradient_accumulation_steps) == 0 or step == len(train_dataloader) - 1:
                    ...
                    if args.kd:
                        logger.info(
                            f"Logit loss {logit_loss.item()}\t"
                            f"Hidden state loss {hs_loss.item()}\t"
                            f"Attention loss {attn_loss.item()}"
                        )
                    
                    if args.quant:
                        logger.info(f"Quant loss {qr_loss}")
                        logger.info(f"Qunat loss details: {qr_loss_details}")
                    
                    if args.clip_reg:
                        logger.info(f"Clip reg loss {clip_reg_loss}")
                        logger.info(f"Clip reg loss details: {clip_reg_loss_details}")
        ...
        if args.quant:
            remove_qr_hooks(unwrapped_model)

        if args.clip_reg:
            remove_clip_reg_hooks(unwrapped_model)
    else:
        perplexity = evaluation(model, eval_dataloader)
